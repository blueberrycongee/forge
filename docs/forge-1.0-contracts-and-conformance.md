...
| --- | --- | --- | --- | --- | --- |
| Event protocol | Ordered metadata + stable event shapes | `src/runtime/event.rs` unit tests | Partial | Add cross-run JSON compatibility golden tests | P0 |
| Checkpoint/resume | Pause/resume preserves state and emits lifecycle events | `tests/integration/pause_resume.rs` | Partial | Add multi-interrupt and invalid resume command cases | P0 |
| Permission flow | ask/allow/deny semantics with replies | `tests/integration/permission_flow.rs`, `tests/integration/tool_context_permission.rs` | Partial | Add precedence and wildcard conformance table tests | P0 |
| Tool lifecycle | tool start/update/result/error sequencing | `tests/unit/tool_interface.rs` | Partial | Add strict ordering assertions for all terminal branches | P0 |
...
| Snapshot/log format | serialize/deserialize compatibility | `src/runtime/session.rs` unit tests | Partial | Add versioned fixture tests (v1 -> latest) | P0 |
...
2. Add golden fixtures for event, checkpoint, and snapshot serialization.
3. Enforce CI gate: conformance suite must pass before merge.

## Phase 1 (2-4 weeks): Conformance Expansion

1. Add table-driven permission and state-machine edge case tests.
2. Add tool lifecycle ordering tests (including cancel/abort paths).
