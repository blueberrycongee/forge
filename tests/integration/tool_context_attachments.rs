...
fn tool_emits_attachment_events() {
    ...
    registry.register("emit", Arc::new(|_call, _ctx| {
        Box::pin(async move {
    ...
    let node = LoopNode::with_tools("loop", Arc::clone(&registry), |mut state: AttachmentState, ctx| async move {
    ...
    assert!(events.iter().any(|event| matches!(event, Event::ToolAttachment { .. })));
}
