...
/// Stream node function signature
///
/// A stream node can emit runtime events while executing.
pub type StreamNodeFn<S> =
    Arc<dyn Fn(S, Arc<dyn EventSink>) -> BoxFuture<'static, GraphResult<S>> + Send + Sync>;
...
pub struct NodeSpec<S: GraphState> {
    ...
    pub stream_func: Option<StreamNodeFn<S>>,
    /// Optional metadata
...
impl<S: GraphState> NodeSpec<S> {
    ...
    /// Create a new stream-capable node
    pub fn new_stream<F, Fut>(name: impl Into<String>, func: F) -> Self
    where
        F: Fn(S, Arc<dyn EventSink>) -> Fut + Send + Sync + 'static,
        Fut: Future<Output = GraphResult<S>> + Send + 'static,
    {
        let name = name.into();
        let stream_func: StreamNodeFn<S> = Arc::new(move |state, sink| Box::pin(func(state, sink)));
        let stream_func_clone = Arc::clone(&stream_func);
        let func = Arc::new(move |state| {
            let sink: Arc<dyn EventSink> = Arc::new(NoopEventSink);
            (stream_func_clone)(state, sink)
        });
        Self {
            name,
            func,
            stream_func: Some(stream_func),
            metadata: None,
        }
    }
    ...
    /// Attach a stream function to an existing node
    pub fn with_stream<F, Fut>(mut self, func: F) -> Self
    where
        F: Fn(S, Arc<dyn EventSink>) -> Fut + Send + Sync + 'static,
        Fut: Future<Output = GraphResult<S>> + Send + 'static,
    {
...
impl<S: GraphState> Clone for NodeSpec<S> {
    fn clone(&self) -> Self {
        Self {
            ...
            stream_func: self.stream_func.clone(),
