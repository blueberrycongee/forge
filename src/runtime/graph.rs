...
use crate::runtime::constants::{has_reserved_chars, is_reserved_name, END, START};
...
impl<S: GraphState> StateGraph<S> {
    ...
    where
        F: Fn(S, std::sync::Arc<dyn crate::runtime::event::EventSink>) -> Fut
            + Send
            + Sync
            + 'static,
        Fut: Future<Output = GraphResult<S>> + Send + 'static,
    {
        ...
        self.nodes
            .insert(name.clone(), NodeSpec::new_stream(name, func));
        self
    }
